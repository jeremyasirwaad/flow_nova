...
export default function UserApprovalNode({ data }: NodeProps) {
  ...
  const getBorderColor = () => {
    switch (executionState) {
      case "executing":
        return "border-amber-400 border-4";
      case "completed":
    ...
    <div className={`bg-white rounded-2xl shadow-lg border-2 ${getBorderColor()} ${getAnimationClass()} min-w-[200px] relative`}>
      <div className="flex items-center gap-3 p-4">
        <div className="w-10 h-10 rounded-xl bg-amber-100 flex items-center justify-center flex-shrink-0 relative">
          ...
          <svg
            className={`w-5 h-5 text-amber-600 ${executionState === "executing" ? "opacity-30" : ""}`}
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
        ...
        <div className="flex-1">
          ...
          {getStatusMessage() && (
            <div className={`text-xs mt-0.5 font-medium ${
              executionState === "executing" ? "text-amber-600" : "text-green-600"
            }`}>
              {getStatusMessage()}
            </div>
          )}
          {data.description && !getStatusMessage() && (
      ...
      {/* Output Handles - Yes (top) and No (bottom) */}
      <Handle
        type="source"
